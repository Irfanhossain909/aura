...
class AppImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Image.asset(
        networkPlaceholderImage ?? AssetsIconsPath.instance.extentionBg,
        width: width,
        height: height,
        fit: fit,
        color: iconColor,
...
class _FullScreenImageViewerState extends State<FullScreenImageViewer>
  ...
  Matrix4 _applyZoom() {
    ...
    const translationCorrection =
        2 - 1; // This is effectively 1, which can be removed for clarity.
...
class _NetworkImageWithRetryState extends State<NetworkImageWithRetry>
  ...
  Widget _loadingPlaceholder() {
    return Skeletonizer(
      enabled: true,
      containersColor: AppColors.instance.blue,
      child: Container(
        width: widget.width ?? double.infinity,
        height: widget.height,
        color: AppColors.instance.white,
        child: Image.asset(
          AssetsIconsPath.instance.extentionBg,
          fit: BoxFit.fill,
        ),
      ),
    );
  ...
  Widget _errorPlaceholder() {
    return Container(
      width: widget.width,
      height: widget.height,
      color: AppColors.instance.navyBlue.withValues(alpha: .5),
      child: Center(
        child: Icon(Icons.image_not_supported, color: AppColors.instance.white),
      ),
    );
  }
